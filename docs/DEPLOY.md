...
| H5 前端 | `dist/`（`npm run build:h5`） | — | 静态站点，需**单独用 1Panel 网站托管** |
...
### 1) 前端（**改了要重新 build**）
- **`src/constants/env.ts` → `API_BASE`**：前端调用的**后端地址**（H5 与小程序共用）。上线改成线上后端域名（如 `https://api.your-domain.com`），并确认 `USE_MOCK=false`。改后 `npm run build:h5` / `build:weapp`。
- **`config/index.ts` → `h5.publicPath`**：H5 静态资源基路径，默认 `/`。若 H5 部署在子路径（如 `/h5/`）改成 `/h5/`。

### 2) 后端 / 后台（改 `.env` 或后台设置，**无需重新构建前端**）
- **`backend/.env` → `CORS_ORIGINS`**：H5 与后端**不同源**时，必须把 H5 域名加进来，否则浏览器 CORS 拦截。例：`["https://admin.your-domain.com","https://h5.your-domain.com"]`。若 H5 与 API 同域反代则可省。
...
docker compose up -d --build              # 起 后端 + 后台
...
H5 不在 docker-compose 里（只有 backend + admin），需**单独构建 + 托管**：
```bash
npm ci
npm run build:h5        # 产物在 dist/
```
把 `dist/` 部署到 1Panel「网站 → 静态网站」根目录，绑 H5 域名 + HTTPS。
如需自动化，可在 `scripts/auto-deploy.sh` 部署成功后追加 `npm ci && npm run build:h5 && rsync -a --delete dist/ <web根>`（需服务器装 Node）。

> 小程序端（`build:weapp`）经微信开发者工具上传发布。

---

## 生产上线检查单

- [ ] `backend/.env`：`DEBUG=false`、随机 `JWT_SECRET`、强 `ADMIN_PASSWORD`、真实 `DATABASE_URL`、`CORS_ORIGINS` 含 H5 + 后台域名
- [ ] `src/constants/env.ts`：`API_BASE` 指向线上后端、`USE_MOCK=false` → 重新 build
