...
13. **H5 没有 `page` 元素**：H5 页面容器是 `div.taro_page`，且 Taro 的 postcss-html-transform 只把 `view/text/button` 等小程序标签映射成 `taro-*-core`，**不会**把 `page` 映射成 `body`。所以全局样式必须写 `page, body { ... }`，否则 H5 端底色/文字色整个失效（白底黑字）。反向在小程序端 `body` 会被编译成 `.h5-body`（无匹配元素），无副作用。
